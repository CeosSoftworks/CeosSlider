...
CEOS.Slider.Transitions.SlideBothRightDelay = new CEOS.Slider.Transition({
	title: "Slide both right with delay",
	
	init: function(el, dur, sender) {
		new CEOS.Slider.Animation(
			'ceos-trans-slide-both-right-delay-in',
			[CEOS.Slider.getVendorCSSPrefix() + 'transform: translateX(-100%)'],
			[CEOS.Slider.getVendorCSSPrefix() + 'transform: translateX(0%)']
		);
		
		new CEOS.Slider.Animation(
			'ceos-trans-slide-both-right-delay-out',
			[CEOS.Slider.getVendorCSSPrefix() + 'transform: translateX(0%)'],
			[CEOS.Slider.getVendorCSSPrefix() + 'transform: translateX(100%)']
		);
	},
	...
	transitionIn: function(el, dur, sender) {
		...
		el.style[animation + 'Name'] 						= 'ceos-trans-slide-both-right-delay-in'
	...
	transitionOut: function(el, dur, sender) {	
		...
		el.style[animation + 'Name'] 						= 'ceos-trans-slide-both-right-delay-out'
		el.style[animation + 'Duration'] 				= dur / 1000 + 's';
